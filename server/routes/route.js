...
router.get("/usercomplaint/:id",(req,res)=>{
    res.render('complaints_user.ejs',{username:req.params.id})
})
...
router.post("/complaintvehicle/:username",async(req,res)=>{
    x=req.body
    const newdata={
        username:req.params.username,
        vehiclename:x.vehiclename,
        complaint:x.vehicleissue
    }
   Complaints.create(newdata).then(()=>{
    setTimeout(function() {
        res.redirect(`/usercomplaint/${req.params.username}`)}, 2000);   
})
    .catch((err)=>{console.log(err)})
})

router.post("/complaintothers/:username",async(req,res)=>{
    x=req.body
    const newdata={
        username:req.params.username,
        complaint:x.otherissue
    }
   Complaints.create(newdata).then(()=>{
    setTimeout(function() {
        res.redirect(`/usercomplaint/${req.params.username}`)}, 1000);   
})
    .catch((err)=>{console.log(err)})
})
/*admin*/
...
router.get("/admin/complaints",(req,res)=>{
    Complaints.find({}).exec().then((result)=>{
        res.render('complaints_admin.ejs',{data:result})
    })
})
router.get('/deletecomplaint/:username/:complaint',(req,res)=>{
    Complaints.deleteOne({username:req.params.username,complaint:req.params.complaint}).exec().then(()=>{
